...
// ==========================================================================
// CTCSOL KNOWLEDGE - SEPARATE NAMESPACE TO AVOID CONFLICTS
// ==========================================================================
const ctcsol = {
    // Data storage
    defaultEntries: [
        {
            zh: '母语规则负迁移',
            vi: 'Sai lệch quy tắc tiếng mẹ đẻ',
            en: 'L1 rule negative transfer',
            topic: 'Transfer',
            mode: 'Transfer',
            explain: 'Người học áp dụng sai quy tắc của tiếng mẹ đẻ sang tiếng đích, gây lỗi.',
            example: 'Tiếng Việt: "tôi đi học" có chủ ngữ đầu. Tiếng Anh: người Việt nói "I go study" thay vì "I go to study".'
        },
        {
            zh: '学习策略的影响',
            vi: 'Tác động của chiến lược học tập',
            en: 'Impact of learning strategies',
            topic: 'Learning',
            mode: 'Strategy',
            explain: 'Cách người học sử dụng kỹ thuật hoặc phương pháp học tập ảnh hưởng đến hiệu quả học.',
            example: 'Học từ qua flashcard và đọc sách sẽ tạo kết quả ghi nhớ khác nhau.'
        },
        {
            zh: '文化因素负迁移',
            vi: 'Sai lệch yếu tố văn hóa',
            en: 'Negative cultural transfer',
            topic: 'Culture',
            mode: 'Transfer',
            explain: 'Hiểu lầm hoặc áp dụng sai chuẩn mực văn hóa tiếng mẹ đẻ sang tiếng đích.',
            example: 'Người Việt nói thẳng trong tiếng Anh có thể gây thô lỗ, trong khi tiếng Anh yêu cầu tính diplomacy.'
        }
    ],

    storeKey: 'ctcsolKnowledgeEntries_v1',
    entries: [],
    selectedIndex: 0,
    flashIndex: 0,
    editingId: null,
    ...
    init() {
        console.log('Initializing CTCSOL Knowledge...');
        this.entries = this.loadEntries();
        this.setupEventListeners();
        this.setupFilters();
        this.render();
        if (this.entries.length) this.selectEntry(this.entries[0]);
    ...
    duplicateKey(entry) {
        return String(entry.zh || '').trim().toLowerCase();
    },

    reviewDefaults() {
        return { lastReviewed: null, dueDate: null, interval: 0, repetitions: 0, ease: 2.5 };
    },
    ...
    normalizeEntry(entry) {
        const review = this.reviewDefaults();
        return {
            id: entry.id || Date.now() + '_' + Math.random().toString(16).slice(2),
            zh: entry.zh || entry['Tiếng Trung'] || entry['tieng_trung'] || entry.chinese || '',
            vi: entry.vi || entry['Tiếng Việt'] || entry['tieng_viet'] || entry.vietnamese || '',
            en: entry.en || entry['Tiếng Anh'] || entry['tieng_anh'] || entry.english || '',
            topic: entry.topic || entry.Topic || entry['Chủ đề'] || entry.mode || 'General',
            mode: entry.mode || entry.Mode || entry['Loại'] || 'Other',
            sheet: entry.sheet || entry.Sheet || '',
            explain: entry.explain || entry['Giải thích'] || entry.explanation || '',
            example: entry.example || entry['Ví dụ'] || entry.examples || '',
            lastReviewed: entry.lastReviewed || review.lastReviewed,
            dueDate: entry.dueDate || review.dueDate,
            interval: Math.max(0, this.toNumber(entry.interval, review.interval)),
            repetitions: Math.max(0, this.toNumber(entry.repetitions, review.repetitions)),
            ease: Math.max(1.3, this.toNumber(entry.ease, review.ease))
        };
    },
    ...
    loadEntries() {
        try {
            ...
            if (Array.isArray(saved) && saved.length) return saved.map(e => this.normalizeEntry(e));
        } catch {}
        return this.defaultEntries.map(e => this.normalizeEntry(e));
    },

    saveEntries() {
        localStorage.setItem(this.storeKey, JSON.stringify(this.entries.map(e => this.normalizeEntry(e))));
    },
    ...
    dueTime(entry) {
        ...
        const time = new Date(entry.dueDate).getTime();
        return Number.isFinite(time) ? time : 0;
    },

    isDue(entry) {
        return this.dueTime(entry) <= this.todayStart().getTime();
    },

    dueEntries() {
        return this.filteredEntries()
            .filter(e => this.isDue(e))
            .sort((a, b) => this.dueTime(a) - this.dueTime(b));
    },

    reviewLabel(entry) {
        if (!entry.lastReviewed) return 'Not reviewed yet';
        const due = entry.dueDate ? new Date(entry.dueDate).toLocaleDateString() : 'today';
        return `${entry.repetitions || 0} repetition(s) · interval ${entry.interval || 0} day(s) · due ${due}`;
    },

    setupFilters() {
        const filterSelect = this.$('ctcsol-categoryFilter');
        filterSelect.innerHTML = '<option value="All">All topics</option>';
        this.unique(this.entries.map(e => e.topic)).forEach(topic => {
        ...
        const quickChipsDiv = this.$('ctcsol-quickChips');
        quickChipsDiv.innerHTML = ['All', ...this.unique(this.entries.map(e => e.topic))].map(topic => (
            `<button class="ctcsol-chip${topic === 'All' ? ' active' : ''}" data-topic="${this.escapeHTML(topic)}">${this.escapeHTML(topic)}</button>`
        )).join('');

        document.querySelectorAll('.ctcsol-chip').forEach(chip => {
            chip.onclick = () => {
                this.$('ctcsol-categoryFilter').value = chip.dataset.topic;
                this.render();
            };
        });
    },
    ...
    filteredEntries() {
        const query = this.$('ctcsol-searchInput').value.trim().toLowerCase();
        const topic = this.$('ctcsol-categoryFilter').value;
        return this.entries.filter(entry => {
            const text = [entry.zh, entry.vi, entry.en, entry.explain, entry.example, entry.topic, entry.mode, entry.sheet].join(' ').toLowerCase();
            return (!query || text.includes(query)) && (topic === 'All' || entry.topic === topic);
        });
    },

    filteredTableEntries() {
        const query = this.$('ctcsol-tableSearchInput').value.trim().toLowerCase();
        return this.entries.filter(entry => {
            const text = [entry.zh, entry.vi, entry.en, entry.topic, entry.explain, entry.example].join(' ').toLowerCase();
    ...
    selectEntry(entry) {
        ...
        this.$('ctcsol-detailZh').textContent = entry.zh;
        this.$('ctcsol-detailVi').textContent = entry.vi;
        this.$('ctcsol-detailEn').textContent = entry.en;
        this.$('ctcsol-detailExplain').textContent = entry.explain;
        this.$('ctcsol-detailExample').textContent = entry.example;
        document.querySelectorAll('.ctcsol-term-card').forEach(card => {
            card.classList.toggle('active', Number(card.dataset.index) === this.selectedIndex);
        });
    },

    fillForm(entry) {
        this.$('ctcsol-zhInput').value = entry.zh || '';
        this.$('ctcsol-viInput').value = entry.vi || '';
        this.$('ctcsol-enInput').value = entry.en || '';
        this.$('ctcsol-topicInput').value = entry.topic || '';
        this.$('ctcsol-explainInput').value = entry.explain || '';
        this.$('ctcsol-exampleInput').value = entry.example || '';
    },

    resetForm() {
        this.editingId = null;
        this.$('ctcsol-formTitle').textContent = 'Manage CTCSOL Data';
        this.$('ctcsol-saveBtn').textContent = 'Save Term';
        this.$('ctcsol-cancelEditBtn').style.display = 'none';
        this.fillForm({});
        this.$('ctcsol-formPanel').open = false;
    },

    saveTerm() {
        const existing = this.editingId ? this.entries.find(entry => entry.id === this.editingId) : null;
        const payload = this.normalizeEntry({
            id: this.editingId || undefined,
            zh: this.$('ctcsol-zhInput').value.trim(),
            vi: this.$('ctcsol-viInput').value.trim(),
            en: this.$('ctcsol-enInput').value.trim(),
            topic: this.$('ctcsol-topicInput').value.trim() || 'General',
            mode: existing?.mode || 'Other',
            sheet: existing?.sheet || '',
            explain: this.$('ctcsol-explainInput').value.trim(),
            example: this.$('ctcsol-exampleInput').value.trim(),
        ...
        if (!payload.zh || !payload.vi) return alert('Cần ít nhất Tiếng Trung và Tiếng Việt.');
        const duplicate = this.entries.find(entry => entry.id !== this.editingId && this.duplicateKey(entry) === this.duplicateKey(payload));
        if (duplicate) return alert(`Trùng Tiếng Trung: ${payload.zh}`);
        if (this.editingId) {
            this.entries = this.entries.map(entry => entry.id === this.editingId ? payload : entry);
        } else {
    ...
    editEntry(id) {
        ...
        if (!entry) return;
        this.editingId = id;
        this.$('ctcsol-formTitle').textContent = 'Edit Term';
        this.$('ctcsol-saveBtn').textContent = 'Save Changes';
        this.$('ctcsol-cancelEditBtn').style.display = 'inline-block';
        this.$('ctcsol-formPanel').open = true;
        this.fillForm(entry);
    ...
    render() {
        ...
        this.$('ctcsol-totalStat').textContent = this.entries.length;
        this.$('ctcsol-topicStat').textContent = this.unique(this.entries.map(entry => entry.topic)).length;
        this.$('ctcsol-visibleStat').textContent = visible.length;
        this.$('ctcsol-dueStat').textContent = this.dueEntries().length;

        document.querySelectorAll('.ctcsol-chip').forEach(chip => {
            chip.classList.toggle('active', chip.dataset.topic === this.$('ctcsol-categoryFilter').value);
        });

        this.$('ctcsol-termGrid').innerHTML = visible.length ? visible.map(entry => `
            <article class="ctcsol-term-card${this.entries.indexOf(entry) === this.selectedIndex ? ' active' : ''}" data-index="${this.entries.indexOf(entry)}">
                <div>
                    <div class="ctcsol-zh">${this.escapeHTML(entry.zh)}</div>
                    <div class="ctcsol-meta">${this.escapeHTML(entry.topic)}</div>
                </div>
                <div>
                    <div class="ctcsol-vi">${this.escapeHTML(entry.vi)}</div>
                    <div class="ctcsol-en">${this.escapeHTML(entry.en)}</div>
                    <div class="ctcsol-meta">${this.escapeHTML(entry.explain)}</div>
                    <div class="ctcsol-meta">${this.escapeHTML(this.isDue(entry) ? 'Due now' : this.reviewLabel(entry))}</div>
                    <div class="ctcsol-term-actions">
                        <button onclick="event.stopPropagation(); ctcsol.editEntry('${this.escapeHTML(entry.id)}')">Edit</button>
                        <button class="ctcsol-danger" onclick="event.stopPropagation(); ctcsol.deleteEntry('${this.escapeHTML(entry.id)}')">Delete</button>
                    </div>
                </div>
            </article>
        `).join('') : '<div class="ctcsol-empty">Không có thuật ngữ khớp bộ lọc.</div>';

        document.querySelectorAll('.ctcsol-term-card').forEach(card => {
            card.onclick = () => this.selectEntry(this.entries[Number(card.dataset.index)]);
        });

        if (visible.length && !visible.includes(this.entries[this.selectedIndex])) this.selectEntry(visible[0]);
        this.renderDataTable();
    },

    renderDataTable() {
        const rows = this.filteredTableEntries();
        this.$('ctcsol-dataTableBody').innerHTML = rows.length ? rows.map(entry => `
            <tr>
                <td>${this.escapeHTML(entry.zh)}</td>
                <td>${this.escapeHTML(entry.vi)}</td>
                <td>${this.escapeHTML(entry.en)}</td>
                <td>${this.escapeHTML(entry.topic)}</td>
            </tr>
        `).join('') : '<tr><td colspan="4">Không có dòng nào khớp bộ lọc.</td></tr>';
    },

    renderFlashcard() {
        if (!this.entries.length) {
            this.currentFlashId = null;
            this.$('ctcsol-flashPrompt').textContent = 'Chưa có dữ liệu';
            this.$('ctcsol-flashMeta').textContent = 'Import hoặc thêm term mới';
            this.$('ctcsol-reviewMeta').textContent = 'No review queue';
            this.$('ctcsol-flashAnswer').textContent = '';
            return;
        }
        const queue = this.dueEntries();
        const source = queue.length ? queue : this.filteredEntries();
        const entry = source[this.flashIndex % source.length] || this.entries[0];
        this.currentFlashId = entry.id;
        this.$('ctcsol-flashcard').classList.remove('show');
        this.$('ctcsol-flashPrompt').textContent = entry.zh;
        this.$('ctcsol-flashMeta').textContent = entry.topic || 'General';
        this.$('ctcsol-reviewMeta').textContent = this.reviewLabel(entry);
        this.$('ctcsol-flashAnswer').innerHTML = `
            <div class="ctcsol-vi">${this.escapeHTML(entry.vi)}</div>
            <div class="ctcsol-en">${this.escapeHTML(entry.en)}</div>
            <p style="margin-top:10px;">${this.escapeHTML(entry.explain)}</p>
            <p class="ctcsol-meta">${this.escapeHTML(entry.example)}</p>
        `;
    },

    currentFlashEntry() {
        return this.entries.find(entry => entry.id === this.currentFlashId) || null;
    },

    scheduleReview(entry, quality) {
        let repetitions = Number(entry.repetitions || 0);
        let interval = Number(entry.interval || 0);
        let ease = Number(entry.ease || 2.5);
        if (quality < 3) {
            repetitions = 0;
            interval = 0;
        } else {
            repetitions += 1;
            if (repetitions === 1) interval = quality === 3 ? 1 : 2;
            else if (repetitions === 2) interval = quality === 3 ? 3 : 6;
            else interval = Math.max(1, Math.round(interval * ease));
            if (quality === 3) interval = Math.max(1, Math.round(interval * 0.65));
            if (quality === 5) interval = Math.max(interval + 1, Math.round(interval * 1.3));
        }
        ...
        return {
            ...
            dueDate: this.addDays(this.todayStart(), interval).toISOString(),
            interval,
            repetitions,
            ease
        };
    ...
    rateCurrentCard(quality) {
        if (!this.currentFlashId) return;
        this.entries = this.entries.map(entry => entry.id === this.currentFlashId ? this.scheduleReview(entry, quality) : entry);
        this.saveEntries();
        this.flashIndex = 0;
        this.render();
        this.renderFlashcard();
    ...
    exportCSV() {
        const headers = ['zh', 'vi', 'en', 'topic', 'mode', 'sheet', 'explain', 'example', 'lastReviewed', 'dueDate', 'interval', 'repetitions', 'ease'];
        const rows = this.entries.map(entry => headers.map(header => this.csvEscape(entry[header])).join(','));
        this.downloadFile('ctcsol-knowledge.csv', [headers.join(','), ...rows].join('\n'), 'text/csv;charset=utf-8');
    },

    exportJSON() {
        this.downloadFile('ctcsol-knowledge.json', JSON.stringify(this.entries, null, 2), 'application/json;charset=utf-8');
    },
    ...
    parseDelimited(text) {
        ...
        }).filter(entry => entry.zh || entry.vi || entry.en);
    },
        ...
        reader.onload = () => {
            try {
                ...
                const imported = file.name.toLowerCase().endsWith('.json')
                    ? JSON.parse(text).map(e => this.normalizeEntry(e))
                    : this.parseDelimited(text);
                if (!imported.length) return alert('Không tìm thấy dòng dữ liệu nào để import.');
                const known = new Set(this.entries.map(e => this.duplicateKey(e)));
                const seenInFile = new Set();
                const cleanImport = [];
                let skipped = 0;
                imported.forEach(entry => {
                    const key = this.duplicateKey(entry);
                    if (!key || known.has(key) || seenInFile.has(key)) {
                        skipped += 1;
                        return;
                    }
                    seenInFile.add(key);
                    cleanImport.push(entry);
                });
                if (!cleanImport.length) return alert(`Không có dòng mới để import. Đã bỏ qua ${skipped} dòng trùng Tiếng Trung.`);
                this.entries = [...cleanImport, ...this.entries];
                this.saveEntries();
                this.setupFilters();
                this.resetForm();
                this.render();
                ...
                this.renderFlashcard();
                alert(`Đã import ${cleanImport.length} thuật ngữ mới. Bỏ qua ${skipped} dòng trùng Tiếng Trung.`);
            } catch (error) {
                alert('Import lỗi. Hãy kiểm tra file CSV/TSV/JSON.');
            }
        };
    ...
    setupEventListeners() {
        ['ctcsol-searchInput', 'ctcsol-categoryFilter'].forEach(id => {
            const el = this.$(id);
            if (el) el.addEventListener('input', () => this.render());
        });
        const tableSearch = this.$('ctcsol-tableSearchInput');
        if (tableSearch) tableSearch.addEventListener('input', () => this.renderDataTable());

        const saveBtn = this.$('ctcsol-saveBtn');
        if (saveBtn) saveBtn.onclick = () => this.saveTerm();

        const cancelBtn = this.$('ctcsol-cancelEditBtn');
        if (cancelBtn) cancelBtn.onclick = () => this.resetForm();

        const exportCsvBtn = this.$('ctcsol-exportCsvBtn');
        if (exportCsvBtn) exportCsvBtn.onclick = () => this.exportCSV();

        const exportJsonBtn = this.$('ctcsol-exportJsonBtn');
        if (exportJsonBtn) exportJsonBtn.onclick = () => this.exportJSON();

        const importBtn = this.$('ctcsol-importBtn');
        const importFile = this.$('ctcsol-importFile');
        if (importBtn) importBtn.onclick = () => importFile?.click();
        if (importFile) {
            importFile.onchange = event => {
                const file = event.target.files[0];
                if (file) this.importEntries(file);
                event.target.value = '';
            };
        }

        const resetBtn = this.$('ctcsol-resetBtn');
        if (resetBtn) {
            resetBtn.onclick = () => {
                if (!confirm('Reset về dữ liệu mẫu trong draft?')) return;
                this.entries = this.defaultEntries.map(e => this.normalizeEntry(e));
                this.saveEntries();
                this.setupFilters();
                this.resetForm();
                this.render();
        ...
        const showAnswerBtn = this.$('ctcsol-showAnswerBtn');
        if (showAnswerBtn) showAnswerBtn.onclick = () => this.$('ctcsol-flashcard').classList.toggle('show');

        const nextBtn = this.$('ctcsol-nextBtn');
        if (nextBtn) {
            nextBtn.onclick = () => {
                this.flashIndex += 1;
                this.renderFlashcard();
        ...
        // Rating buttons
        document.querySelectorAll('#ctcsol-ratingActions button').forEach(button => {
            button.onclick = () => this.rateCurrentCard(Number(button.dataset.quality));
        });
...
// Initialize CTCSOL when the tab is opened
document.addEventListener('DOMContentLoaded', () => {
    const ctcsolTab = document.querySelector('[data-tab="tab-ctcsol"]');
    if (ctcsolTab) {
        ctcsolTab.addEventListener('click', () => {
            if (!ctcsol.entries.length) {
                ctcsol.init();
            }
        });
    }
...
// Initialize CTCSOL if tab is visible at startup
if (document.querySelector('#tab-ctcsol.active')) {
    ctcsol.init();
}
